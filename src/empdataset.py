...
class EmpDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = albumentations.Resize(
            height=224, width=224, interpolation=3, always_apply=True)(image=image)['image']
        ...
        if cfg.MODEL == 'Inception':
            # resize to 160x160
            image = albumentations.Resize(
                height=160, width=160, interpolation=3, always_apply=True)(image=image)
            image = image['image']
