...
def df_check(df_train, df_valid):
    """Checks for data leaks from training data to validation data in the generated excel

    Args:
        df_train ([dataframe object]): 
        df_valid ([dataframe object]):
    """
    print('*'*50)
    print(f'[INFO] Checking Dataframes for missing emp names in validation set..')
    print('*'*50)

    diff_empnames = set(df_train[df_train.columns[1]]) - \
        set(df_valid[df_valid.columns[1]])
    if diff_empnames is not None:
        print(
            f'[INFO] Validation dataset has no - {diff_empnames}  emperors from training set')
    else:
        print(
            f'[INFO] Validation dataset has no - {diff_empnames}  emperors from training set- Stratified')
    ...
    data_leak_check = set(df_train.ImagePath).intersection(
        set(df_valid.ImagePath))
    if not data_leak_check:
        print(f'[INFO] No data leak from validation set to training set...')
    else:
        print(
            f'[INFO] Data leak from validation set to training set... - {data_leak_check}')
        print('Breaking..')
        exit
...
def load_datasets(dataset_path, excel_path, augmentations=None, training_data=False):
    ...
    print('*'*50)
    dataset = empdataset.EmpDataset(
        dataset_path, excel_path, augmentations=augmentations)
    dataset.print_info()
    print('-'*50)
    print('\n')
...
def separate_bn(model):
    ...
    paras_wo_bn = list(
        filter(lambda p: id(p) not in paras_only_bn_id, all_params))
...
if __name__ == '__main__':
    ...
    if cfg.AUGMENTATION:
        ...
        aug = Compose([HorizontalFlip(p=0.5),
                       ...
                       MultiplicativeNoise(
                           multiplier=[0.8, 1], elementwise=True, per_channel=True, p=0.8)
                       ])
    else:
        aug = None
    ...
    train_dataset = load_datasets(
        dataset_path, train_excel_path, aug, training_data=True)
    valid_dataset = load_datasets(
        dataset_path, valid_excel_path, None, training_data=False)
    ...
    cols = cfg.COLUMNS
    train_samples_count = get_samples_per_cls(
        train_dataset.df[cols[1]])  # column = label from the train.xlsx
    valid_samples_count = get_samples_per_cls(
        valid_dataset.df[cols[1]])  # column = label from the valid.xlsx
    ...
    valid_cls_weights = get_cls_weights(
        valid_samples_count, effective_weights=True)
    train_cls_weights = get_cls_weights(
        train_samples_count, effective_weights=True)
    ...
    if cfg.WEIGHTED_SAMPLING:
        ...
        weighted_sampler = get_weighted_sampler(
            train_samples_count, train_dataset.df[cols[1]])
    ...
    train_loss_fn = LossFunctions(
        loss_fn_name=cfg.LOSS_FN, alpha=1, gamma=cfg.GAMMA, cls_weights=None)
    valid_loss_fn = LossFunctions(
        loss_fn_name=cfg.LOSS_FN, alpha=1, gamma=cfg.GAMMA, cls_weights=None)
