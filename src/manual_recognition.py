...
def manual_face_recog(path, img_size=256, save_in=None, resize=False, bgr2rgb=False, align=False):
    ...
    print(points)
    print(
        f'[INFO] Face - {bbox},left_eye - {left_eye}, right_eye - {right_eye}')
    ...
    for box in boxes:
        cropped_face = FaceDetector.get_face(
            aligned_img, box, image_size=img_size, margin=0, save_path=None)
...
if __name__ == "__main__":
    ...
    ap.add_argument("-i", "--imagename", required=False,
                    help="Image name..eg: keizer (1).jpg")
    ap.add_argument("-s", "--saveas", required=False,
                    help='new image name if required..')
