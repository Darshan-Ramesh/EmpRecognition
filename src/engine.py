...
def precision(logits,y):
    _,y_preds = torch.max(logits,1)
    pscore = precision_score(y_preds.cpu().numpy(),y.cpu().numpy(),average='weighted')
    return pscore

def recall(logits,y):
    _,y_preds = torch.max(logits,1)
    rscore = recall_score(y_preds.cpu().numpy(),y.cpu().numpy(),average='weighted')
    return rscore
...
def pass_epoch(model,model_head,loss_fn,data_loader,
    ...
    for i_batch, (x,y,_) in enumerate(data_loader):
        ...
        if writer is not None and model.training:
            if writer.iteration % writer.interval == 0:
                writer.add_scalars('BatchLoss',{mode: loss_batch.detach().cpu()},epoch)
                
                for metric_name, metric_batch in metrics_batch.items():
                    writer.add_scalars(metric_name, {mode: metric_batch},epoch)
            writer.iteration += 1
    ...
    if model.training and scheduler is not None:
        scheduler.step()
        writer.add_scalars('learning rate', {mode: lr(optimizer)},writer.iteration)
